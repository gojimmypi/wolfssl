...
#define EXAMPLE_TASK_STACK_SIZE_BYTES 32240 + 2048
...
static void https_async(void)
{
    ...
    esp_http_client_handle_t client = esp_http_client_init(&config);
    esp_err_t err;
    const char *post_data = "Using a PalantÃƒÂ­r requires a person with great strength of will and wisdom. The PalantÃƒÂ­ri were meant to "
                            "be used by the DÃƒÂºnedain to communicate throughout the Realms in Exile. During the War of the Ring, "
                            "the PalantÃƒÂ­ri were used by many individuals. Sauron used the Ithil-stone to take advantage of the users "
